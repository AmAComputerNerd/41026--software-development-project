...
// UserDto is what we hand back to clients from the auth endpoints.
// PasswordHash is intentionally [JsonIgnore]-ed so it never leaves
// the service. The contract record carries the password hash from
// the database service for the login path, but the public DTO
// never serializes it.
