...
- Each bounded context owns an independent database — EF Core over either SQLite or PostgreSQL.
...
| **`student-1-backend`** | `student-1/backend` | ASP.NET Core (.NET 10) + EF Core (Npgsql) | `5101` | `http://student-1-backend:8080` | Notifications, delivery preferences, AI digests & chat, SSE stream, Canvas sync (`/notifications/*`, `/digest/*`, `/preferences/*`, `/api/canvas-sync`) |
| **`student-1-database`** | `student-1/database` | PostgreSQL 16 Alpine | `5432` | `student-1-database:5432` | Student 1 persistence (`notifications_db`), seeded by `init.sql` |
...
| **`student-2-backend`** | `student-2/backend` | ASP.NET Core (.NET 10) + SQLite | `5102` | `http://student-2-backend:8080` | Automation CRUD/run history, scheduled Canvas posts, AI quiz filling, assignment extensions (`/api/automations`, `/api/automation-runs`) |
...
| **`student-3-backend`** | `student-3/backend` | ASP.NET Core (.NET 10) | `5103` | `http://student-3-backend:8080` | Public deadline/task API, Canvas and AI orchestration, due-soon reminders (`/api/tasks/*`, `/api/courses/*`, `/api/canvas-sync`) |
...
| **`student-4-backend`** | `student-4/backend` | ASP.NET Core (.NET 10) | `5104` | `http://student-4-backend:8080` | User, student, teacher, and AI profile-summary APIs (`/api/users/*`, `/api/students/*`, `/api/teachers/*`) |
| **`student-4-authentication`** | `student-4/authentication` | ASP.NET Core (.NET 10) | `5114` | `http://student-4-authentication:8080` | Login, password/account management, and email password reset (`/api/auth/*`) |
| **`student-4-database`** | `student-4/database` | ASP.NET Core (.NET 10) + EF Core SQLite | *Internal only* | `http://student-4-database:8080` | Exclusive owner of Student 4 persistence and migrations |
| **`student-4-frontend`** | `student-4/frontend` | Vue 3 + TypeScript + Vite | *Proxied* | `http://student-4-frontend:80` | Login, profile, and password-reset UI (`/account/`) |
...
| **`student-5-database`** | `student-5/database` | ASP.NET Core (.NET 10) + EF Core SQLite | *Internal only* | `http://student-5-database:8080` | Exclusive owner of Student 5 persistence and migrations |
...
│   ├── backend/                       # NotificationService ASP.NET Core + SSE broker + Api.Tests
│   ├── database/                      # PostgreSQL 16 container + init.sql seeding
│   └── frontend/                      # Vue 3 Notifications UI + AI Digest Assistant + Playwright e2e
│
├── student-2/                         # Automations vertical slice (Isaac Thomas)
│   ├── backend/                       # Automation API, execution worker, EF Core/SQLite
│   └── frontend/                      # Automation configuration and run history UI
│
...
│   ├── backend/                       # Public profile API and AI summaries
│   ├── authentication/                # Authentication and password-reset API
│   ├── database/                      # Internal EF Core/SQLite persistence service
│   ├── contracts/                     # Shared internal persistence contracts
│   └── frontend/                      # Account, login, profile and reset UI
│
├── student-5/                         # Grades & Progress vertical slice (William Hannah)
│   ├── backend/                       # GradesManager ASP.NET Core public API
│   ├── database/                      # Internal EF Core/SQLite persistence service
│   ├── contracts/                     # Shared internal persistence contracts
...
| `ConnectionStrings__DefaultConnection` | `student-1-backend` | PostgreSQL connection string for `student-1-database` |
...
| `DatabaseService__BaseUrl` | `student-3`, `student-4` (API and authentication), `student-5` | Private persistence API URL |
...
2. **Automation Execution**: `student-2-backend` periodically claims enabled automations from its SQLite database → uses `shared-backend` for Canvas courses, recipients, conversations, and quizzes → uses `ai-mode` when an automation requires AI-generated quiz answers → records the run result locally.
3. **Account and Profile Management**: `/api/auth/*` requests go to `student-4-authentication`, while `/api/users/*`, `/api/students/*`, and `/api/teachers/*` go to `student-4-backend` → both services access `student-4-database` over its private HTTP API; password resets send email through MailHog, and profile-summary generation calls `ai-mode`.
4. **Proactive Deadline Reminders**: `student-3-backend` queries due candidates from `student-3-database` → dispatches `POST /notifications/push` to `student-1-backend` → records successful delivery through the database service.
...
- [Playbook: Split API and Database Services](docs/playbooks/split-database-service.md)
